#!/bin/sh
# gitsync uninstaller — removes everything install.sh put in place, except
...
# gitsync binary is still around to do so.
...
if [ -L "$BIN_PATH" ] || [ -f "$BIN_PATH" ]; then
  rm -f "$BIN_PATH"
  echo "gitsync: removed $BIN_PATH"
fi

if [ -d "$SHARE_DIR" ]; then
  rm -rf "$SHARE_DIR"
  echo "gitsync: removed $SHARE_DIR"
fi
...
for rc in "${ZDOTDIR:-$HOME}/.zshrc" "$HOME/.bash_profile" "$HOME/.bashrc" "$HOME/.profile"; do
  ...
  ' "$rc" > "$tmp"
  mv "$tmp" "$rc"
  echo "gitsync: removed PATH line from $rc"
done

cat <<EOF

gitsync has been uninstalled. Your config and targets were left in place:
  ${XDG_CONFIG_HOME:-$HOME/.config}/gitsync/
